...
"""
    parse(io | string) -> order, ngrams

Parse an ARPA-formatted `io` object/string and return a array of
dictionary for the ngrams. Returns the order of the language model
and dictionary storing the ngram, weights and backoffs:
`ngram => (weight, backoff)`.
"""
        ...
        elseif state == body
            ...
            elseif startswith(stripped_line, "\\$(length(ngrams) + 1)-grams:")
                ...
                if 2 <= length(tokens)  <= 3
                    ...
                    record = (
                        log(10f0) * parse(Float32, tokens[1]),
                        length(tokens) == 3 ? log(10f0) * parse(Float32, tokens[3]) : missing
                    )
